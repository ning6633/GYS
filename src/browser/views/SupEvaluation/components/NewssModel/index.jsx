...
class NewssModel extends React.Component {
  ...
  //选择资质申请fn
    ...
    async loadZzzs(pageNum = 1, rowNum = 10,name){
        ...
        //资质评价专家load分页查询
        async PjzjpageChange(page, num) {
            this.loadPjzj(page, num)
        }
        //资质评价专家搜索
        async PjzjSearch(value){
            this.loadPjzj(1, 10,value)
        }
        //加载评价专家
        async loadPjzj(pageNum = 1, rowNum = 10,username){
            ...
            if(specialist){
                this.setState({
                    specialist:{
                        list:specialist.listZzpjSpecialistVO,
                        recordsTotal:specialist.recordsTotal
                    },
                    Pjzjpaginations: {search:(value)=>{this.PjzjSearch(value)}, showTotal:()=>`共${specialist.recordsTotal}条`, onChange: (page, num) => { this.PjzjpageChange(page, num) }, showQuickJumper: true, total: specialist.recordsTotal, pageSize: 10 }

                })
            }
            // let PjzjList = await supplierEvalution.getZzpjCertificateAll(pageNum,rowNum,params)
            // console.log(PjzjList)
            // if(PjzjList){
            //     this.setState({
            //         PjzjList:{
            //             list:PjzjList.data.listZzpjCertificateVO,
            //             recordsTotal:PjzjList.data.recordsTotal
            //         },
       ...
       let listModelOption={
        ...
        columns:[
            ...
            },
            {
                ...
                dataIndex: 'apply',
                align: "center",
                render: (text, record) => <span style={{ cursor: "pointer", 'color': '#3383da' }}>{`航天资质申请-${record.admittance_grade}` }</span>
            },
            {
                title: '供应商名称',
                dataIndex: 'gys_name',
    ...
    this.setState({
        ...
        if(userInfo.code==200){
            ...
            setFieldsValue({
                doquaevalorg:userInfo.data[0].departmentname
            })
        }
    ...
    render() {
        ...
        const {BzyqData,PjzjData,AccessSupData,ReferData,SQList,ZzzsList,specialist,listModelOption,cerlistModelOption,paginations,Zzzspaginations,speaciallistModelOption,Pjzjpaginations  } =  this.state
        ...
        const Bzyqcolumns = [
            ...
            },
            {
                title: '要求类型',
                dataIndex: 'type',
                render: (text, record, key) =>record.type1 && <span>{record.type1}</span>
             
            },
            {
                title: '标准文件编号',
                dataIndex: 'number',
                render: (text, record, key) =>record.code && <span>{record.code}</span>
            },
            {
                title: '标准文件附件',
                dataIndex: 'attach',
                render: (text, record) =>record.fileid && <span style={{ cursor: "pointer", 'color': '#3383da' }} onClick={()=>{window.open(`${supplierEvalution.FileBaseURL}${record.fileid}`) }}> 查看附件 </span>
            },
            {
                title: '操作',
                dataIndex: 'cz',
        ...
        return (
            <div>
                  ...
                  {/* 选择证书 */}
                {
                 ...
                 {/* 选择专家 */}
                 {
                    toggleStore.toggles.get(SHOW_ChooseSpecialist_MODEL)&&<ChooseListModel list={specialist} pagination={Pjzjpaginations} comparedList={PjzjData} options={speaciallistModelOption}  chooseFinishFn={(val)=>{this.chooseSpecialistFn(val)}} />
                }
                  {/* {
