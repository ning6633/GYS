...
class SignUp extends React.Component {
   ...
   async componentDidMount(){
    ...
    async handleSubmit() {
        //点击确定出发的事件
        let { toggleStore,id, info ,loaddata} = this.props
        let { addList } = this.state
      
        if(info.status=='2'){
          let res = await supplierTrain.gysZRApplysignUp(id)
           console.log(res)
           if(res.code == 200 ){
    ...
    async trainApplyNew(id,data) {
        ...
        let res = await supplierTrain.addUserToZRTrainApply(id, _arr)
        if(res.code == 200){
             message.success(res.message)
        }
    }
    async getAddedList(applyId){
        let { addList} = this.state
        let params = {
            applyId,
            pageNum:1,
        ...
        let res = await supplierTrain.getTrainApplyUsers(params)
       console.log(res)
        if(res.code==200){
            let _arr =res.data
...
let res = await supplierTrain.deleteTrainApplyNewUsers([record.id])
    ...
    render = () => {
        ...
        return (
            <Modal
                title={<div style={{ width: "100%", textAlign: "center", fontWeight: 900 }}>确认报名人员</div>}
