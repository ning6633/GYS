...
@inject('toggleStore')
@observer
class TrainingApplyZR extends Component {
    state = {
        trainPlanList: {
            list: [],
            recordsTotal: 0
    ...
    async loaddata(pageNum = 1, rowNum = 20) {
        ...
        let trainPlanList = userTypeVerty == "approve" ? await supplierTrain.getTrainApplyNew4PJZR(params) : await supplierTrain.getPlansOfGys(userId, pageNum, rowNum, searchValue);
    ...
    const { toggleStore } = this.props;
        ...
        switch (type) {
            case 'applyed':
                ...
                result = await supplierTrain.getTrainZRApplied(record.trainid)
                break;
            case 'waited':
                title = '待审批供应商'
                result = await supplierTrain.getTrainZRapprovalApply(record.trainid)
            ...
            case 'unApply':
                ...
                case 'userApplyed':
                title = '已申请总人数'
                result = await supplierTrain.getTrainZRAppliedPerson(record.trainid)
...
TrainingApplyZR.propTypes = {
}
export default Form.create({ name: 'TrainingApplyZR' })(TrainingApplyZR);