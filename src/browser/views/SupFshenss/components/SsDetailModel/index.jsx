...
class SsDetailModel extends React.Component {
  ...
  chooseSpecialistFn(data){
    ...
    async loadGys(pageNum=1,rowNum=10){
        const { annuaRecordid} = this.state
        let gysResult = await supYearAudit.getGYSBySs(pageNum,rowNum,annuaRecordid)
        console.log(gysResult)
        if(gysResult.code==200){
            let gysData = gysResult.data.list||[]
            let AccessSupData = [],ReferData = []
            console.log(AccessSupData)
            gysData.forEach(item => {
                if(item.status=='1'){
    ...
    async loadSpeciallist(pageNum=1,rowNum=10){
        const { annuaRecordid} = this.state
        let ZjResult = await supYearAudit.loadSpeciallist(pageNum,rowNum,'',annuaRecordid)
        console.log(ZjResult)
        if(ZjResult.code==200){
            let PjzjData = ZjResult.data.list||[]
            this.setState({
                PjzjData,
            })
        }
                    ...
                    <Tabs defaultActiveKey="1" size={'large'}>
                        <TabPane tab="评价信息" key="1">
                            <Form className="ant-advanced-search-form" onSubmit={(e) => {this.handleSubmit(e) }}>
                                        ...
                                        <Col span={24}>
                                            ...
                                            <Col span={12} >
                                                <Form.Item {...formItemLayout} label="评价时间">
                                                        {getFieldDecorator('annualauditDate')(<Input disabled />)}
                                                </Form.Item>
                                            </Col>
                                            <Col span={12} >
                                                <Form.Item {...formItemLayout} label={'评价地点'}>
                                                    {getFieldDecorator(`annualauditAddress`)(<Input disabled />)}
                                                </Form.Item>
                                            </Col>
                                            <Col span={12} >
                                                <Form.Item {...formItemLayout} label={'满意度'}>
                                                    {getFieldDecorator(`score`)(<Input disabled/>)}
                                                </Form.Item>
                                            </Col>
                                        </Col>
                                    </Row>
                                ...
                                <Card bordered={false} title={<b>标准要求</b>} className="new_supplier_producelist">
                                    <Row>
                                        <Col span={24}>
                                            <Table rowKey={(text, key) => text.id} columns={Bzyqcolumns} dataSource={BzyqData} />
                                        </Col>
                                    </Row>
                                </Card>
                                ...
                                className="new_supplier_producelist">
                                    <Row>
                                        <Col span={24}>
                                            <Table rowKey={(text, key) => text.id} columns={Pjzjcolumns} dataSource={PjzjData} />
                                        </Col>
                                    </Row>
                                </Card>
                            </Form>
                        ...
                        <TabPane tab="供应商" key="2">
                            <Card bordered={false} title={<b>已通过供应商</b>}  className="new_supplier_producelist">
                                <Row>
                                    <Col span={24}>
                                        <Table rowKey={(text, key) => text.provider_id} scroll={{x:1100}} columns={AccessSupcolumns} dataSource={AccessSupData} />
                                    </Col>
                                </Row>
                            </Card>
                            <Card bordered={false} title={<b>未通过供应商</b>} className="new_supplier_producelist">
                                <Row>
                                    <Col span={24}>
                                        <Table rowKey={(text, key) => text.provider_id} scroll={{x:1100}} columns={ReferSupcolumns} dataSource={ReferData} />
                                    </Col>
                                </Row>
                            </Card>
                        </TabPane>
