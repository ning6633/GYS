...
class SupplierDirectory extends Component {
    ...
    async getproviderbyproducttype(categoryId = '', pageNum = 1, rowNum = 20) {
        let res = await supplierDirectory.getgysinfos({ categoryId, pageNum, rowNum })
        if (res.code == 200) {
            let flag = 0;
    ...
    render() {
        let { producttype, upload, treeData, selectNode, tableData, pageNum, rowNum, loading, directoriesId,point } = this.state
        return (
            <Layout title={"供应商名录管理"}>
                <Row gutter={24}>
                    <Col className="gutter-row" span={5}>
                        <div className="gutter-box">
                            <SupDirectManager getProducttype={this.getProducttype} getSubCategories={this.getSubCategories} upload={upload} treeData={treeData} selectNode={selectNode} onSelectTree={this.onSelectTree} onSelect={this.onSelect} />
                        </div>
                    </Col>
                    <Col className="gutter-row" span={19}>
                        <div className="gutter-box">
                            <SupplierDirectoryInfo producttype={producttype} tableData={tableData} pageNum={pageNum} rowNum={rowNum}
                                pageChange={this.pageChange} loading={loading} selectNode={selectNode} onSelectedRowKeys={this.onSelectedRowKeys}
