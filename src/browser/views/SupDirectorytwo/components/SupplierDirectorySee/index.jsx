...
class SupplierDirectorySee extends Component {
    state = {
        ...
        point: false,
        directoriesId: '',
        orgid: supplierDirectory.pageInfo.departmentId,
        typecode: '5',
        keyword: '',
    }
    ...
    // 按名录ID获取供应商
    async getproviderbyproducttype(categoryId = '', orgid = '', typecode = '', keyword = '', pageNum = 1, rowNum = 20) {
        let res = await supplierDirectory.getGysInfosByCategoryId({ categoryId, orgid, typecode, keyword, pageNum, rowNum })
        if (res.code == 200) {
            let flag = 0;
    ...
    pageChange = (pageNum, rowNum) => {
        let { producttype, orgid, typecode, keyword } = this.state
        this.setState({
            pageNum,
            rowNum,
            loading: true,
        }, () => {
            this.getproviderbyproducttype(producttype, orgid, typecode, keyword, pageNum, rowNum)
        })
    }
    ...
    // 搜索状态
    searchValue = (orgid, typecode, keyword) => {
        let { producttype, pageNum, rowNum } = this.state
        this.setState({
            orgid,
            typecode,
            keyword,
    ...
    // 下拉选择
    selectSearchValue = (orgid, typecode, keyword) => {
        this.setState({
    ...
    render() {
        let { producttype, upload, treeData, selectNode,onSelectInfo, tableData, pageNum, rowNum, loading, directoriesId, point, orgid, typecode, keyword } = this.state
        return (
                    ...
                    <Col className="gutter-row" span={19}>
                        <div className="gutter-box">
                            <SupplierDirectoryInfo producttype={producttype} tableData={tableData} pageNum={pageNum} rowNum={rowNum}
                                pageChange={this.pageChange} loading={loading} selectNode={selectNode} onSelectedRowKeys={this.onSelectedRowKeys} searchValue={this.searchValue} orgid={orgid} typecode={typecode} keyword={keyword}
