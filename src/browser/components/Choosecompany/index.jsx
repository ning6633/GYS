...
class ChooseCompany extends React.Component {
    ...
    async searchCompanyInfo(name) {
        if (name) {
            let ret = await supplierAction.searchCompanyInfo(name);
            this.setState({
                isSearch: true,
                treedata: ret
            })
    ...
    async componentDidMount() {
        let ret = await supplierAction.getCompanyListTree()
        this.setState({
            treedata: ret
        })
    }
    ...
    render() {
        ...
        return (
            <div>
                <Icon style={{ cursor: 'pointer' }} onClick={() => toggleStore.setToggle(SHOW_ChooseCompany_MODEL)} type="plus" />
                <Modal
                    title="选择院所"
