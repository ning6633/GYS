...
import { SHOW_ChooseTBSupperlier_MODEL } from "../../constants/toggleTypes"
...
@inject('toggleStore')
@observer
class ChooseTBSupplier extends React.Component {
    state = {
        supplierList: [],
    ...
    handleOk = e => {
        const { toggleStore,chooseTBsupplierFn ,supplierList} = this.props;
        const { selectedRowKeys } = this.state
        let selectData = _.find(supplierList, { id: selectedRowKeys[0] })
        chooseTBsupplierFn(selectData);
        toggleStore.setToggle(SHOW_ChooseTBSupperlier_MODEL)
    };
    ...
    render() {
        ...
        return (
            <div>
                <Icon style={{ cursor: 'pointer' }} onClick={() => toggleStore.setToggle(SHOW_ChooseTBSupperlier_MODEL)} type="plus" />
                <Modal
                    title="选择已填报供应商"
                    visible={toggleStore.toggles.get(SHOW_ChooseTBSupperlier_MODEL)}
                    width={800}
                    centered
                    okText="确认"
                    cancelText="取消"
